      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
              ...
              child: Container(
                ...
                child: Column(
                  children: [
                    ...
                    /// CATEGORY
                    Obx(() {
                      final category = controller.itemDetails.value?['category'] ?? 
                                      product?['category'] ?? 
                                      'Top';
                      return Container(
                    ...
                    /// TITLE
                    Obx(() {
                      final title = controller.itemDetails.value?['title'] ?? 
                                   product?['title'] ?? 
                                   product?['name'] ?? 
                    ...
                    /// DESCRIPTION
                    Obx(() {
                      final description = controller.itemDetails.value?['description'] ?? 
            ...
            SizedBox(height: 16.h),
            // Purchase Items - Dynamic from API
            Padding(
              padding: EdgeInsets.symmetric(horizontal: 20.w),
              child: Obx(() {
                // Show loading
                ...
                return Column(
                  children: List.generate(
                    ...
                    (index) {
                      ...
                      return Padding(
                        padding: EdgeInsets.only(bottom: 16.h),
                        child: _buildDynamicPurchaseItem(
                          product: product,
                          index: index,
                          isFavorited: isFavorited,
                          onFavoriteToggle: () => controller.toggleProductFavorite(index),
                          onAddToCart: () => controller.addToCart(index),
                          onBuyNow: () => controller.buyNow(index),
                        ),
                      );
                    },
                  ),
...
Widget _buildWardrobeItem(Map<String, dynamic> item) {
  // Get image path - try different field names from API
...
// Dynamic Purchase Item Widget
Widget _buildDynamicPurchaseItem({
